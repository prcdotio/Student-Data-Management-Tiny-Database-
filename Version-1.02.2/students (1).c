    ...
    while(choice == 'y' || choice == 'Y'){
        ...
        switch(search_option){
            case 1:
            {
                ...
                case 2:
                {
                    int found_index = search_by_name(storage, size, "edit");
                    if(found_index != -1){
                        storage = edit(storage, size, found_index);
                    }
                }
...
int search_by_name(struct students *storage, int *size, char status[]){
    ...
    for (int i = 0; i < *size; ++i)
    {
        ...
        if(match_count == 1){
            printf("Only one student found named '%s'. Proceeding...\n", search_name);
            return match_indices[0];
        }
        int selected_id;
        printf("Multiple matches. Please select ID to proceed %s.\n", status);
        ...
        printf("Error: The ID %d was not found on search list.\n", selected_id);
        return -1;
}
    ...
    while(choice == 'y' || choice == 'Y'){
        ...
        switch(search_option){
            case 1:
            {
                ...
                case 2:
                {
                    int found_index = search_by_name(storage, size, "delete");
                    if(found_index != -1){
                        storage = remove_data(storage, size, found_index);
                    }
                }
